  ...
  <div class="body">
    <div class="media" v-for="gathering in gatherings" v-bind:key=gathering.id>
      <div class="media-body">
        <h6 class="mt-0">
          <a href="#">{{ gathering.name | titleEllipsis}}</a>
        </h6>
        <span class="contain-font-size">{{ gathering.summary | ellipsis }}</span>
        <br />
        <span class="media-contain">
          •
          主办方：<span>{{gathering.sponsor}}</span>&nbsp;&nbsp;&nbsp;
          •
          活动开始日期：<span>{{gathering.startTime | formatDate}}</span>&nbsp;&nbsp;&nbsp;
          •
          截止日期：<span>{{gathering.endTime | formatDate}}</span>
        </span>
